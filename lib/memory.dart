...
class MemoryState extends State<Memory> {
  ...
  void _initBoard() async {
    ...
    setState(() => _isLoading = true);_data = map;
    if (_data.length == 2 || _data.length == 3 || _data.length == 4 || _data.length == 5 || _data.length == 6 ||
        _data.length == 7) {
      _maxSize = 2;
    } else {_maxSize = 8;}
    ...
    _allLetters = [];
    _data.forEach((k, v) {
      _allLetters.add(k);_allLettersUpperCase.add(k);_allLetters.add(v);_allLettersLowerCase.add(v);
    });
    ...
    _shuffledLetters = [];
    _shuffledLetters.addAll(_allLetters.take(_size * _size).toList(growable: false)..shuffle());
    ...
    _shaker = [];_shaker = _letters.map((a) => ShakeCell.Right).toList(growable: false);
